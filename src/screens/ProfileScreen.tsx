...
export default function ProfileScreen({ onNavigate, activeTab }: Props) {
  ...
  const [units, setUnits] = useState(() => localStorage.getItem('gerakfit-units') || 'kg')
  ...
  function toggleUnits() {
    ...
    setUnits(next)
    localStorage.setItem('gerakfit-units', next)
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0d0d0d', fontFamily: 'system-ui, sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: '#0d0d0d', borderBottom: '0.5px solid #1a1a1a', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          ...
          <button onClick={() => {
            if (navigator.share) {
              navigator.share({ title: 'GerakFit', text: 'Check out my GerakFit profile!', url: window.location.href })
            }
      ...
      {/* Settings overlay */}
      {section === 'settings' && (
        <div style={{ position: 'fixed', inset: 0, background: '#0d0d0d', zIndex: 200, overflowY: 'auto' }}>

          {/* Header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', borderBottom: '0.5px solid #1a1a1a' }}>
            <button onClick={() => { setSection('main'); setShowThemePicker(false) }} style={{ background: 'none', border: 'none', color: '#888', fontSize: '16px', cursor: 'pointer' }}>← Back</button>
            <div style={{ flex: 1, textAlign: 'center', fontSize: '17px', fontWeight: 700, color: '#fff' }}>Settings</div>
          ...
          {[
            {
              group: 'Account',
              items: [
                { icon: '👤', label: 'Edit Profile', action: () => setSection('edit_profile'), danger: false, extra: undefined as string | undefined },
                { icon: '⚖️', label: 'Body Log', action: () => setSection('body_log'), danger: false, extra: undefined as string | undefined },
              ]
            },
            {
              group: 'Preferences',
              items: [
                { icon: '🌙', label: 'Theme', action: () => setShowThemePicker(p => !p), extra: currentThemeLabel, danger: false },
                { icon: '📏', label: 'Units', action: () => toggleUnits(), extra: units, danger: false },
                { icon: '🌐', label: 'Language', action: () => {}, extra: 'English', danger: false },
              ]
            },
            {
              group: 'Data',
              items: [
                { icon: '📤', label: 'Export Data', action: () => console.log('export'), danger: false, extra: undefined as string | undefined },
                { icon: '📥', label: 'Import Data', action: () => console.log('import'), danger: false, extra: undefined as string | undefined },
              ]
            },
            {
              group: 'Help',
              items: [
                { icon: '❓', label: 'Help & Support', action: () => {}, danger: false, extra: undefined as string | undefined },
                { icon: 'ℹ️', label: 'About GerakFit', action: () => alert('GerakFit v1.0.0'), danger: false, extra: undefined as string | undefined },
              ]
            },
            {
              group: 'Danger',
              items: [
                { icon: '🚪', label: 'Sign Out', action: () => signOut(), danger: true, extra: undefined as string | undefined },
              ]
            },
          ].map(grp => (
            <div key={grp.group}>
              <div style={{ fontSize: '11px', color: '#555', textTransform: 'uppercase', letterSpacing: '1px', padding: '16px 20px 8px', background: '#0d0d0d' }}>
                {grp.group}
              </div>
              <div style={{ background: '#1c1c1e' }}>
                {grp.items.map((item, i) => (
                  <div key={item.label} onClick={item.action} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', cursor: 'pointer', borderBottom: i < grp.items.length - 1 ? '0.5px solid #1a1a1a' : 'none' }}>
                    <span style={{ fontSize: '18px', width: '28px', textAlign: 'center' }}>{item.icon}</span>
                    <span style={{ flex: 1, fontSize: '15px', color: item.danger ? '#ef4444' : '#fff' }}>{item.label}</span>
                    {item.extra && <span style={{ fontSize: '13px', color: '#555', marginRight: '4px' }}>{item.extra}</span>}
                    {!item.danger && <span style={{ fontSize: '18px', color: '#444' }}>›</span>}
                  </div>
                ))}
              </div>
              {grp.group === 'Preferences' && showThemePicker && (
                <div style={{ background: '#111', borderBottom: '0.5px solid #1a1a1a' }}>
                  {['System Default', 'Light', 'Dark'].map(t => (
                    <div key={t} onClick={() => applyTheme(t)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 20px 12px 60px', borderBottom: '0.5px solid #1a1a1a', cursor: 'pointer' }}>
                      <span style={{ fontSize: '14px', color: '#ccc' }}>{t}</span>
                      {currentThemeLabel === t && <span style={{ color: '#1D9E75', fontSize: '16px' }}>✓</span>}
                    </div>
                  ))}
                </div>
              )}
