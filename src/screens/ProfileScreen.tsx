...
export default function ProfileScreen({ onBack }: Props) {
  ...
  const P: React.CSSProperties = { minHeight:'100vh', background:darkMode?'#111827':'#f9fafb', fontFamily:'system-ui,sans-serif', paddingBottom:'32px' }
  const H: React.CSSProperties = { background:darkMode?'#1f2937':'#fff', borderBottom:'1px solid #e5e7eb', padding:'14px 16px', display:'flex', alignItems:'center', gap:'12px' }
  const B: React.CSSProperties = { padding:'20px 16px', maxWidth:'500px', margin:'0 auto' }
  const CARD: React.CSSProperties = { background:darkMode?'#1f2937':'#fff', border:`1px solid ${darkMode?'#374151':'#e5e7eb'}`, borderRadius:'14px', padding:'16px 18px', marginBottom:'14px' }
  const CT: React.CSSProperties = { fontSize:'13px', fontWeight:600, color:darkMode?'#f9fafb':'#111827', margin:0 }
  const FG: React.CSSProperties = { marginBottom:'18px' }
  const LBL: React.CSSProperties = { display:'block', fontSize:'13px', fontWeight:500, color:darkMode?'#d1d5db':'#374151', marginBottom:'8px' }
  const INP: React.CSSProperties = { width:'100%', padding:'10px 12px', borderRadius:'8px', border:`1px solid ${darkMode?'#374151':'#e5e7eb'}`, fontSize:'14px', color:darkMode?'#f9fafb':'#111827', boxSizing:'border-box', fontFamily:'system-ui,sans-serif' }
  ...
  if (loading) return <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'system-ui,sans-serif' }}><div style={{ fontSize:'13px', color:darkMode?'#6b7280':'#9ca3af' }}>Loading...</div></div>

  if (section === 'edit_profile') return (
    <div style={P}>
      <div style={H}><button onClick={()=>setSection('main')} style={BTN}>← Back</button><div style={{ fontSize:'16px', fontWeight:600, color:darkMode?'#f9fafb':'#111827', flex:1 }}>Edit profile</div>{saveMsg&&<div style={SM}>{saveMsg}</div>}</div>
      <div style={B}>
        ...
        <div style={FG}><label style={LBL}>Injury notes <span style={{ fontWeight:400, color:darkMode?'#6b7280':'#9ca3af' }}>(optional)</span></label><textarea value={profile.injury_notes} onChange={e=>setProfile(p=>({...p,injury_notes:e.target.value}))} rows={3} style={{ ...INP, resize:'none' }} placeholder="e.g. Left knee pain" /></div>
  ...
  if (section === 'edit_equipment') return (
    <div style={P}>
      <div style={H}><button onClick={()=>setSection('main')} style={BTN}>← Back</button><div style={{ fontSize:'16px', fontWeight:600, color:darkMode?'#f9fafb':'#111827', flex:1 }}>Equipment ({totalSel} selected)</div>{saveMsg&&<div style={SM}>{saveMsg}</div>}</div>
      <div style={B}>
        <div style={{ fontSize:'13px', color:darkMode?'#9ca3af':'#6b7280', marginBottom:'16px' }}>Tap category to expand. Select equipment available at your gym.</div>
        {equipCats.map((cat, ci) => {
          ...
          return (
            <div key={cat.category} style={{ background:darkMode?'#1f2937':'#fff', border:`1px solid ${darkMode?'#374151':'#e5e7eb'}`, borderRadius:'12px', marginBottom:'8px', overflow:'hidden' }}>
              <div onClick={()=>toggleCat(cat.category)} style={{ padding:'12px 14px', display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                  ...
                  <span style={{ color:darkMode?'#6b7280':'#9ca3af', fontSize:'12px' }}>{expanded?'▲':'▼'}</span>
                </div>
              </div>
  ...
  if (section === 'body_log') return (
    <div style={P}>
      <div style={H}><button onClick={()=>setSection('main')} style={BTN}>← Back</button><div style={{ fontSize:'16px', fontWeight:600, color:darkMode?'#f9fafb':'#111827', flex:1 }}>Body weight log</div></div>
      <div style={B}>
        <div style={CARD}>
        ...
        <div style={CARD}>
          ...
          {bodyLogs.length===0?<div style={{ textAlign:'center', padding:'20px 0', fontSize:'13px', color:darkMode?'#6b7280':'#9ca3af' }}>No entries yet.</div>:bodyLogs.map((log,i)=>{
            ...
            return (
              <div key={log.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom:'1px solid #f3f4f6' }}>
                <div>
                  ...
                  <div style={{ fontSize:'11px', color:darkMode?'#6b7280':'#9ca3af', marginTop:'1px' }}>{new Date(log.log_date).toLocaleDateString('en-MY',{day:'numeric',month:'short',year:'numeric'})}{log.notes?` · ${log.notes}`:''}</div>
                </div>
                <button onClick={()=>{supabase.from('body_logs').delete().eq('id',log.id);setBodyLogs(p=>p.filter(l=>l.id!==log.id))}} style={{ padding:'4px 10px', borderRadius:'6px', border:'1px solid #fee2e2', background:darkMode?'#1f2937':'#fff', color:'#ef4444', fontSize:'12px', cursor:'pointer' }}>Delete</button>
              </div>
            )
          })}
        </div>
  ...
  return (
    <div style={P}>
      <div style={H}><button onClick={onBack} style={BTN}>← Back</button><div style={{ fontSize:'16px', fontWeight:600, color:darkMode?'#f9fafb':'#111827', flex:1 }}>Profile</div>{saveMsg&&<div style={SM}>{saveMsg}</div>}</div>
      <div style={B}>
        <div style={{ textAlign:'center', marginBottom:'24px' }}>
          ...
          <div style={{ fontSize:'17px', fontWeight:600, color:darkMode?'#f9fafb':'#111827', marginTop:'10px' }}>{profile.full_name||user?.email}</div>
          <div style={{ fontSize:'12px', color:darkMode?'#6b7280':'#9ca3af', marginTop:'2px' }}>{user?.email}</div>
        </div>
        ...
        <div style={CARD}>
          ...
          {[['Goal',goalLabel],['Experience',expLabel],['Training days',`${profile.training_days_per_week} days/week`],['Session length',`${profile.session_length_minutes} min`],['Injuries',profile.injury_notes||'None noted']].map(([l,v])=>(
            <div key={l} style={{ display:'flex', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid #f3f4f6' }}><span style={{ fontSize:'13px', color:darkMode?'#9ca3af':'#6b7280' }}>{l}</span><span style={{ fontSize:'13px', fontWeight:500, color:darkMode?'#f9fafb':'#111827', textAlign:'right', maxWidth:'55%' }}>{v}</span></div>
          ))}
        </div>

        <div style={CARD}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'12px' }}><div style={CT}>Equipment <span style={{ fontWeight:400, color:darkMode?'#6b7280':'#9ca3af' }}>({totalSel} items)</span></div><button onClick={()=>setSection('edit_equipment')} style={EBTN}>Edit</button></div>
          {totalSel===0?<span style={{ fontSize:'13px', color:darkMode?'#6b7280':'#9ca3af' }}>None selected</span>:equipCats.filter(c=>c.items.some(i=>i.selected)).map(c=>(
            <div key={c.category} style={{ marginBottom:'8px' }}>
              <div style={{ fontSize:'11px', fontWeight:500, color:darkMode?'#6b7280':'#9ca3af', textTransform:'uppercase', letterSpacing:'0.4px', marginBottom:'5px' }}>{c.category}</div>
        ...
        <div style={CARD}>
          ...
          {[['Log body weight',()=>setSection('body_log')],['Update equipment',()=>setSection('edit_equipment')]].map(([l,a])=>(
            <div key={l as string} onClick={a as ()=>void} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 0', borderBottom:'1px solid #f3f4f6', cursor:'pointer' }}><span style={{ fontSize:'14px', color:darkMode?'#f9fafb':'#111827' }}>{l as string}</span><span style={{ color:darkMode?'#6b7280':'#9ca3af', fontSize:'16px' }}>›</span></div>
          ))}
        </div>

        <button onClick={()=>signOut()} style={{ width:'100%', padding:'12px', borderRadius:'10px', border:'1px solid #fee2e2', background:darkMode?'#1f2937':'#fff', color:'#ef4444', fontSize:'14px', fontWeight:500, cursor:'pointer', marginTop:'4px' }}>Sign out</button>
      </div>
    </div>
  )
}