    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: darkMode ? '#111827' : '#f9fafb', padding: '24px', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: darkMode ? '#1f2937' : '#fff', border: `1px solid ${darkMode ? '#374151' : '#e5e7eb'}`, borderRadius: '16px', padding: '28px 24px', width: '100%', maxWidth: '380px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div>
              <div style={{ display: 'flex', gap: '4px', marginBottom: '4px' }}>
                {[
                  ...
                  <div key={i} style={{ flex: 1, height: '3px', borderRadius: '2px', background: met ? '#1D9E75' : '#e5e7eb', transition: 'background 0.2s' }} />
                ))}
              </div>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{ marginTop: '4px', width: '100%', padding: '12px', borderRadius: '10px', border: 'none', background: loading ? '#9FE1CB' : '#1D9E75', color: '#fff', fontSize: '14px', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.15s' }}>
            {loading
        ...
        {mode === 'login' && (
          <div style={{ marginTop: '16px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '12px' }}>
              <span onClick={() => switchMode('forgot')} style={{ color: '#9ca3af', cursor: 'pointer' }}>Forgot password?</span>
            </div>
          </div>
        )}
      </div>
